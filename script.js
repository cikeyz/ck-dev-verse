/* --- 1. BOOT SEQUENCE DATA ---
   ...
   I kept them plain so they stay easy to tweak. */
...
commandInput.addEventListener('keydown', function (e) {
  ...
  if (command === 'axiom_start') {
    // If the command is right, I echo it first and then start the fake boot.
    ...
    // Wrong commands still get the bash-style error so the terminal vibe stays the same.
    bootLog.textContent += '> ' + rawCommand + '\n'
...
function printBootLine(index) {
  if (index >= bootLines.length) {
    // Once all lines are done, I fade in the hero text and then the rest of the page.
  ...
  // Blank lines pause a bit less so the boot does not feel too slow.
  setTimeout(function () {
